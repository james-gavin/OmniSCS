...
public class TimeUtils {
    private static ThreadLocal<StringBuilder> mmssBuilder = ThreadLocal.withInitial(StringBuilder::new);
    ...
    public static int parseTime(String time) {
        if (!time.equals("0") && !time.equals("")) {
            ...
            for(int i = 0; i < lifeMatch.length; ++i) {
                for(Matcher matcher = Pattern.compile("([0-9]+)" + lifeMatch[i]).matcher(time); matcher.find();
